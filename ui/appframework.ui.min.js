...
}j=null;var p=!1,q=Object.keys(i).length;if(q>0){p=!0;var r=0;for(var s in i)!function(c){a.ajax({url:intel.xdk.webRoot+i[c],success:function(d){d.length>0&&(b.updatePanel(c,d),b.parseScriptTags(a.query("#"+c).get(0))),r++,r>=q&&(p=!1,a(document).trigger("defer:loaded"))},error:function(){console.log("Error with deferred load "+intel.xdk.webRoot+i[c]),r++,r>=q&&(p=!1,a(document).trigger("defer:loaded"))}})}(s)}if(this.firstDiv){this.activeDiv=this.firstDiv,this.scrollingDivs[this.activeDiv.id]&&this.scrollingDivs[this.activeDiv.id].enable();var t=function(){a.query("#navbar").append(a.create("footer",{id:"defaultNav"}).append(a.query("#navbar").children())),b.defaultFooter="defaultNav",b.prevFooter=a.query("#defaultNav"),b.updateNavbarElements(b.prevFooter);
...
var g=b.getPanelId(c),h=!(g!="#"+b.firstDiv.id);g.length>0&&b.loadDefaultHash&&!h?b.loadContent(c,!0,!1,"none"):(d="#"+b.firstDiv.id,b.firstDiv.style.display="block",b.parsePanelFunctions(b.firstDiv),b.loadContentData(b.firstDiv),a.query("#header #backButton").css("visibility","hidden"),("true"==b.firstDiv.getAttribute("data-modal")||"true"==b.firstDiv.getAttribute("modal"))&&b.showModal(b.firstDiv.id)),b.launchCompleted=!0,a.query("#afui #splashscreen").remove(),setTimeout(function(){a(document).trigger("afui:ready")})};p?a(document).one("defer:loaded",t):t()}a.bind(b,"content-loaded",function(){if(b.loadContentQueue.length>0){var a=b.loadContentQueue.splice(0,1)[0];
...
var c=200,d=this,e={top:this.el.scrollTop,left:this.el.scrollLeft},f=0;d.nativePolling=setInterval(function(){return f++,f>=c?(clearInterval(d.nativePolling),void 0):((d.el.scrollTop!=e.top||d.el.scrollLeft!=e.left)&&(clearInterval(d.nativePolling),a.trigger(a.touchLayer,"scrollend",[d.el]),a.trigger(d,"scrollend",[d.el])),void 0)},20)},h.prototype.hideRefresh=function(b){if(!this.preventHideRefresh){var c=this,e=function(b){c.refreshContainer.style.top="-60px",c.refreshContainer.style.position="absolute",c.dY=c.cY=0,b||(c.el.style[a.feat.cssPrefix+"Transform"]="none",c.el.style[a.feat.cssPrefix+"TransitionProperty"]="none",c.el.scrollTop=0,c.logPos(c.el.scrollLeft,0),c.refreshRunning=!1,c.setRefreshContent("Pull to Refresh"),a.trigger(c,"refresh-finish"))
...
}),this.activeSheet=f,a(c).append('<div id="af_action_mask" style="position:absolute;top:0px;left:0px;right:0px;bottom:0px;z-index:9998;background:rgba(0,0,0,.4)"/>'),setTimeout(function(){var a=f.height();f.vendorCss("Transition","all 300ms"),f.cssTranslate("0,"+-a+"px")},10)};return b.prototype={activeSheet:null,hideSheet:function(){var b=this;this.activeSheet.off("click","a",function(){b.hideSheet()}),a(this.el).find("#af_action_mask").remove(),this.activeSheet.vendorCss("Transition","all 0ms");var c=this.activeSheet,d=this.el;setTimeout(function(){c.vendorCss("Transition","all 300ms"),c.cssTranslate("0,0px"),setTimeout(function(){c.remove(),c=null,d.style.overflow="none"
